...
1. 阻塞，非阻塞：是否需要阻塞*等待*数据就绪；
2. 同步，异步：是否可以干别的事
	1. 同步阻塞：自己不能干别的事
	1. 同步非阻塞：自己不能干别的事，但也不用阻塞等待数据就绪。这又分好几种情况
		1. 同时等待多个io, 但是不阻塞于单个io, 这个就是IO复用(io multiplexing), 典型的如: select/poll/epoll
			2. 严格来说，io复用虽然不阻塞于单个io, 但是将数据从内核复制到用户态缓冲区阶段，还是阻塞的
	1. 异步阻塞：去干的别事，阻塞等待数据(挂起自己)，如：协程, SIGIO 驱动
		1. SIGIO信号驱动，跟io复用一样在内核数据复制时阻塞。
	2. 异步非阻塞：去干别的事, 非阻塞等待数据(也不挂起自己)


1. 同步：数据由内核空间复制回进程缓冲时阻塞(如recvfrom)
	1. 阻塞block：
	2. 非阻塞Non-Block: 就是轮询
	3. IO 复用:主要阻塞在select, poll, epoll（等待可用描述符), 读取数据的I/O系统调用如recvfrom 也是阻塞的
		1. 轮询多个io描述符
			1. select(rlist,wlist,timeout=0)+ 轮询判断FD_ISSET(fd, &rset)，且有FD_SETSIZE 最大1024限制
			2. poll 没有FD_SETSIZE 限制, 但每次收到数据依然需要遍历所有的描述符
		2. 直接返回准备就绪的描述符: epoll、kqueue 事先注册需要检查哪些描述符的哪些事件，当状态发生变化时，内核会调用对应的回调函数，将这些描述符保存下来；下次获取可用的描述符时，*直接返回这些发生变化的描述符*即可。
			3. epoll 也可以叫做Reactor，事件驱动，事件轮循（EventLoop）
				1. 采用的是异步非阻塞回调, 嵌套回调极难维护: 
				1. 没有FD_SETSIZE限制，O(1)时间，N倍并发
				2. libevent是基于epoll系统调用的事件驱动库
			2. kqueue 类似epoll 是mac osx 提供的
	4. IO信号驱动(SIGIO)：信号发生在IO之前，IO由进程完成
...
2. 异步: copy from kernel to buffer, then sig notification, 信号发生在IO之后，IO由内核完成
