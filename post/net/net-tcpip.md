...
2. NAT: 是IP/PORT 转换，基于NAT表(`路由器NAT随机分配的PORT与内网主机ip是一一对应的`)
...
##  MAC地址
同个局域网内是通过mac地址来寻址的，而跨网段通信是通过ip来寻址的，跨网段通信通过路由交换来实现数据的转发，而路由的转发意味着会替换目的mac地址

## 网关和广播
1. Broadcast Address(广播地址)是专门用于同时向局域网络中所有工作站进行发送的一个地址。
2. 网关是内外网数据包必经的ip
在使用TCP/IP 协议的网络中，主机标识段host ID 为全1 的IP 地址为广播地址，广播的分组传送给host ID段所涉及的所有计算机。
    ...
    192.168.1. & 255.255.255. = 192.168.1.
                .169 | ~.0 = .255
...
4. 4位TCP首部长度, 也叫数据偏移 Data Offset: (TCP 协议头长度，最长15: (`4*15=60 bytes`), 如果没有选项字段最短`4*5=20 bytes`),  TCP报文段的*数据起始处* 距离 **TCP报文的起始处** 有多远
4. Reserved: 6 bit 无用
...
6. 16位窗口大小: 2 字节。 告诉对方本端的 TCP 接收缓冲区还能容纳多少字节的数据.(窗口是指数据区大小，实际上选项区还有一个窗口扩大因子)
...
9. 选项+数据
    1. 选项是0~40字节, 4位TCP首部长度减去20就是选项长度. 也可以通过寻找kind=0代表选项结束
...
```
说明：
1. kind=0 选项表结束选项. （1字节长，无长度和数据部分）
2. kind=1 空操作（nop）选项，没有特殊含义. （1字节长，无长度和数据部分）
...
4. kind=3是窗口扩大因子选项。TCP连接初始化时，通信双方使用该选项来协商接收通告窗口的扩大因子。
	1. 在TCP的头部中，接收通告窗口大小是用16位表示的，故最大为65535字节，但实际上TCP模块允许的接收通告窗口大小远不止这个数（为了提高TCP通信的吞吐量）。窗口扩大因子解决了这个问题。假设TCP头部中的接收通告窗口大小是N，窗口扩大因子（移位数）是M，那么TCP报文段的实际接收通告窗口大小是N乘2^M，或者说N左移M位。注意，M的取值范围是0～14。我们可以通过修改/proc/sys/net/ipv4/tcp_window_scaling内核变量来启用或关闭窗口扩大因子选项。
...
21:43:45.360608 IP 106.120.206.249.60987 > 120.24.83.10.81: Flags [S], seq 1244600374, win 65535, options [mss 1386,nop,wscale 5,nop,nop,TS val 802678219 ecr 0,sackOK,eol], length 0
	...
	0x0020:  b002 ffff cc1d 0000 0204 056a 0103 0305  ...........j....
                      b002 :  4位TCP首部长度(1011b: 4*11=44bytes, 所以选项字段占44-20=24字节)+6位保留位+6bit组(URG/ACK/PSH/RST/SYN/FIN)
					  ffff : window: 65535
					  cc16: 16bit检验和=52246
...
注意：IPV6 tcp包更长。
```s
12:20:34.191883 IP6 ::1.57176 > ::1.4500: Flags [S], seq 3822505426, win 65535, options [mss 16324,nop,wscale 6,nop,nop,TS val 860383422 ecr 0,sackOK,eol], length 0
	0x0000:  600e 1098 002c 0640 0000 0000 0000 0000  `....,.@........
	0x0010:  0000 0000 0000 0001 0000 0000 0000 0000  ................
	0x0020:  0000 0000 0000 0001 df58 1194 e3d6 cdd2  .........X......
	0x0030:  0000 0000 b002 ffff 0034 0000 0204 3fc4  .........4....?.
                b002 中02代表SYN
	0x0040:  0103 0306 0101 080a 3348 68be 0000 0000  ........3Hh.....
	0x0050:  0402 0000
```
