...
1. Master 是控制集群的中心:
    ...
    3. Controller : 负责维护集群状态, 比如故障检测、自动扩展、滚动更新
...
2. controller manager 维护集群状态：比如故障检测、自动扩展、滚动更新
3. scheduler 负责资源的调度，按照预定的调度策略将 Pod 调度到相应的机器上；
3. kubelet 负责维护容器的生命周期，同时也负责 Volume（CSI）和网络（CNI）的管理；
...
## k8s 分层架构
1. 核心层(Nucleus API and excution)：
...
1. 接口层(Interface Layer: client libraries and utilities)：
    kubectl 命令行工具、客户端 SDK 以及集群联邦
1. 生态系统：在接口层之上的庞大容器集群管理调度的生态系统，可以划分为两个范畴
    2. Kubernetes 外部：日志、监控、配置管理、CI/CD、Workflow、FaaS、OTS 应用、ChatOps、GitOps、SecOps 等
...
如果安装配置了 FLANNEL_ETCD_PREFIX="/kube-centos/network" 参数(这是 Flannel 查询 etcd 的目录地址)
...
您可以在任何的 pod 或者 pod 的 template 中引用你绑定到 CSI volume 上的 PersistentVolumeClaim(my-request-for-storage)。
