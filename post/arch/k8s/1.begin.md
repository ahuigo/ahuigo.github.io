...
2. Node: 最小硬件节点, 提供 CPU、内存和存储资源的节点。(相当于虚拟机吧), 包括
   1. pod: 类似docker-compose, 可有多个pod, 是部署最小单元
      1. 每个pod可以有自己的虚拟ip
      1. Kubernetes 中部署的最小单位是 pod， 一个 pod 中可以包含一个或多个 Docker 容器. 除非紧密耦合通常一个 pod
         中只有一个容器
      1. docker-compose中的多个containers 放到一个pod中
   2. kubelet : 和master 通信，报告node状态
   3. kube-proxy: 实现集群网络服务的
...
1. 接口层(Interface Layer: client libraries and utilities)： kubectl 命令行工具、客户端 SDK
   以及集群联邦
1. 生态系统：在接口层之上的庞大容器集群管理调度的生态系统，可以划分为两个范畴 2. Kubernetes
   外部：日志、监控、配置管理、CI/CD、Workflow、FaaS、OTS 应用、ChatOps、GitOps、SecOps 等 2.
...
查看 Etcd 中存储的 flannel 网络信息： 如果安装配置了 FLANNEL_ETCD_PREFIX="/kube-centos/network"
参数(这是 Flannel 查询 etcd 的目录地址)
