...
2. pass: standard unix password manager for cli terminal.
    1. brew install pass
3. 用qq/weibo快捷登录：不适合不支持qq/weibo 的网站
...
## pass philosophy
With pass, 
1. each password lives inside of `a gpg encrypted file` whose filename is the `title of the website` 
2. organized into folder hierarchies
4. All passwords live in `~/.password-store/`, tracking password changes using `git`.
...
Here, `gpgid` is the ID of my GPG key`.password-store/.gpg-id`. 
...
### multiple info
This also handles multiline passwords or other data with `--multiline or -m`, and passwords can be edited in your default text editor using `pass edit pass-name`. e.g. For example, `Amazon/bookreader`
