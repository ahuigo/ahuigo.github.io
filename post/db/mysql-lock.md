...
1. read uncommited 脏读：一个事务读取到另一个没有提交的数据
1. read commited, 不可重复读取：第一个事务中的两次读数据之间，由于第二个事务的修改, 读取到不重复的值
1. Repeatable read： 第一个事务中的两次读数据之间，由于第二个事务的修改, 读取不到实际修改的值(幻读)(Phantom Read)
2. 串行读(Serializable)
