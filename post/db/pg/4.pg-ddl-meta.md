...
# namespace
> 一个 namespace 或者 schema 可以用来组织数据库对象。你可以在一个 namespace 中创建对象(库/表/索引)，这样这些对象的名字就只需要在它们所在的 namespace 中是唯一的
> 这使得你可以在不同的 namespace 中有相同名字的表或者其他对象。
...
默认namespace 是public, 查看其它schema的表
